...
def integrating_sso_in_loginpy():
    file_path = "{}/{}".format(BENCH_PATH,"/apps/frappe/frappe/www/login.py")
    with open(file_path, "r") as file:
        content = file.read()
    updated_content = content.replace('''# Copyright (c) 2015, Frappe Technologies Pvt. Ltd. and Contributors
...
@frappe.whitelist(allow_guest=True)
def fetch_agency_token(user_id, password):
    if user_id == "bepc.dept@mail.com" and password == "Department@1234":
        user_id = user_id
        password = password
        start_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        end_time = (datetime.now() + timedelta(minutes=2)).strftime("%Y-%m-%d %H:%M:%S")
        token_data = f"::{user_id}::{start_time}::{end_time}::"
        working_key = "tcilprivatekeyforbepcsso"
...
def redirect_dashboard():
    ...
    working_key = "tcilprivatekeyforbepcsso"
    decrypted_data = decrypt(encoded_ciphertext, working_key)
    user_id = decrypted_data[2:].split("::")[1]
    start_time = datetime.strptime(decrypted_data[2:].split("::")[2], '%Y-%m-%d %H:%M:%S')
    end_time = datetime.strptime(decrypted_data[2:-10].split("::")[3], '%Y-%m-%d %H:%M:%S')
    if user_id == "bepc.dept@mail.com" and end_time > datetime.now():
        frappe.local.login_manager.login_as(user_id)
        redirect_post_login(desk_user=frappe.db.get_value("User", frappe.session.user, "user_type") == "System User", redirect_to="/app")
    else:
        return "Session Time Out !!"
''')
    with open(file_path) as f:
        if '''# Copyright (c) 2015, Frappe Technologies Pvt. Ltd. and Contributors
# License: MIT. See LICENSE
...
def redirect_dashboard():
    ...
    if user_id == "bepc.dept@mail.com" and end_time > datetime.now():
        ...
        return "Session Time Out !!"''' in f.read():
            return
    with open(file_path, "w") as file:
        file.write(updated_content)
        print('''# Copyright (c) 2015, Frappe Technologies Pvt. Ltd. and Contributors
# License: MIT. See LICENSE
