...
frappe.ui.form.on('Teacher Training Attendance Tool', {
		refresh: function(frm) {
			frm.disable_save();
			// frm.set_df_property('group_based_on','options',['Department','Program','Academic Year'])
		},
		...
		load_employees: function(frm) {
			if(frm.doc.date) {
				frappe.call({
					...
					method: "bepc.bepc.doctype.teacher_training_attendance_tool.teacher_training_attendance_tool.get_employees",
			...
			mark_employee_toolbar.find(".btn-mark-present")
				...
				.on("click", function() {
					...
					frappe.call({
						...
						method: "bepc.bepc.doctype.teacher_training_attendance_tool.teacher_training_attendance_tool.mark_employee_attendance",
						args:{
